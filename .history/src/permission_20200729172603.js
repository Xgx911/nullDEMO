  ...
  switch (Number(loginStatus)) {
    ...
    case 1:
      try {
        ...
        console.log(code)
        // 通过code换取token
        store.dispatch('user/loginWechatAuth', code).then(res => {
          console.log(res, 'code成功')
          store.dispatch('user/setLoginStatus', 2)
        })
        // await store.dispatch('user/setLoginStatus', 2)
        // hash
        console.log(window.location.origin + window.location.pathname + window.location.hash, '路劲')
        ...
        console.log(err, '错误', window.location.origin + window.location.pathname + window.location.hash)
